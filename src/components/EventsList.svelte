<script>
  ...
  let today = $state(DateTime.now()); // default to the date deployed, but update it client-side

  let inPast = $derived((event) => {
    return DateTime.fromISO(event.date).valueOf() < today.valueOf();
  });
  let inFuture = $derived((event) => {
    return DateTime.fromISO(event.date).valueOf() >= today.valueOf();
  });

  let upcomingEvents = $derived(EVENTS.filter(inFuture).sort(soonestFirst));
  let pastEvents = $derived(EVENTS.filter(inPast).sort(mostRecentFirst));
