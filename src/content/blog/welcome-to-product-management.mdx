...
This article is going to go through a bunch of concepts that build on each other:

- Where do PMs come from? What backgrounds?
- There are many different subtypes of PM.
...
I don't mean to suggest that PjMs are _worse_ than PdMs. PjMs can add a lot of value to teams! And having someone on the team do project management is helpful. For example on some teams one individual can successfully be both a PdM and a PjM. On other teams the Tech Lead (lead developer) wears the PjM hat for the developer task backlog.

My issue is when someone has the title Product Manager but is then treated like a Project Manager, without the ability to prioritize things, and without being (culturally) allowed to delegate project management tasks.

The mismatch of title and expectations causes a ton of undue stress and strife for Product Managers.
